import { GameResponse } from "@/types/game-type";
import Link from "next/link";
import Image from "next/image";

interface UpcomingGamesProps {
  upcominggames: GameResponse;
}

export default async function Upcominggames({
  upcominggames,
}: UpcomingGamesProps) {
  return (
    <div className="mt-8">
      <h1 className="text-2xl relative text-center md:text-5xl font-heading text-white after:absolute after:left-1/2 after:top-full after:mt-3 after:h-1 after:w-20 after:-translate-x-1/2 after:rounded-full after:bg-(--primary) mb-12">
        Jogos <span className="text-(--primary)">a caminho</span>
      </h1>
      <p className="mb-4 text-zinc-400 text-center">
        Descubra os jogos que estão para serem lançados e fique pronto para a
        compra
      </p>
      <ul className="grid grid-cols-[repeat(auto-fit,minmax(150px,1fr))] gap-2 md:grid-cols-[repeat(auto-fit,minmax(280px,1fr))] md:gap-6 list-none">
        {upcominggames.results
          .filter((game) => game.background_image)
          .map((game) => (
            <li
              key={game.id}
              className="group relative overflow-hidden rounded-2xl aspect-3/4 cursor-pointer bg-[#1a1a2e] shadow-[0_4px_15px_rgba(0,0,0,0.5)] transition-all duration-300 hover:-translate-y-1"
            >
              <div className="relative h-full w-full">
                <Image
                  src={game.background_image}
                  alt={game.name}
                  fill
                  className="object-cover"
                  sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
                />
              </div>

              <div className="absolute inset-0 flex flex-col justify-end bg-linear-to-t from-black/90 via-black/50 to-transparent p-5">
                <h3 className="text-lg font-semibold text-white transition-opacity duration-300 group-hover:opacity-0">
                  {game.name}
                </h3>

                <Link
                  href={`/game/${game.id}`}
                  className="absolute bottom-7 left-8 rounded-lg bg-[#00ff7f] px-4 py-2 font-medium text-white opacity-0 transition-all duration-300 group-hover:opacity-100 cursor-pointer hover:bg-green-300"
                >
                  Ver detalhes
                </Link>
              </div>
            </li>
          ))}
      </ul>
    </div>
  );
}
